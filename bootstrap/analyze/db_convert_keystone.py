"""Keystone DB Converter - uses Keystone assembler instead of NASM.

Keystone may produce different bytes than NASM for the same mnemonic.
This script tests if Keystone can match bytes that NASM couldn't.
"""
...
# Try Keystone first, fallback to NASM
NASM = Path(r'C:\Users\Elias\nasm\nasm-2.16.03\nasm.exe')
TEMP = Path(r'C:\Users\Elias\temp_db_keystone')
...
def test_keystone(expected_bytes: bytes, comment: str) -> str | None:
    variants = generate_variants(comment)
    for v in variants:
        try:
            encoding, count = ks.asm(v)
            if encoding and bytes(encoding) == expected_bytes:
                return v
        except KsError:
            pass
    return None
...
def process_file(seg_path: Path) -> int:
    ...
    for batch_start in range(0, len(targets), batch_size):
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = {
                executor.submit(test_keystone, expected, comment): (line_idx, expected, comment)
...
def main():
    ...
    print(f"\nDONE: Converted {total} lines with Keystone")
